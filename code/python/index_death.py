  ...
  <body>
    <div class="wrapper">
      <header>
        ...
        <p class="view"><a href="/wordcloud.html">Wordcloud</a></p>

        <p class="view">Biographies</p>

        <p class="view"><a href="/bio/index.html"><i>&mdash;Biographies, sorted by birth</i></a></p>
        <p class="view"><a href="/bio/sort_by_death.html"><i>&mdash;Biographies, sorted by death</i></a></p>
...
def main():
    ...
    fnames = [fname for fname in sorted(os.listdir(args.directory)) if fname not in EXCLUDED and fname[-3:] == ".md"]
    ...
    deathYears = [i.split("/")[0] for i in deathYears]
    deathYears = [int(i.split("~")[1]) if "~" in i else int(i) for i in deathYears]
    ...
    f = open("sort_by_death.html", "w")
    f.write(Template(INDEX_TEMPLATE).render(names=markdown, header=header, links=links, dir=os.getcwd().split("timeline")[1], home=find_home()))
    f.close()
